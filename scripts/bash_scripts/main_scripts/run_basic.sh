...
mkdir -p results/baseline results/proportional runs/logs
...
do
    ...
    : > runs/logs/error_basic_${seed}.txt
    tmux send-keys -t patient_basic_${seed} "conda activate ${environment}; python -u all_policies.py --seed ${seed} --n_patients ${N} --n_providers ${M} --provider_capacity 1 --noise 0.25 --num_trials 25 --utility_function semi_synthetic_comorbidity --order uniform --out_folder baseline >> ../../runs/logs/error_basic_${seed}.txt 2>&1" ENTER
